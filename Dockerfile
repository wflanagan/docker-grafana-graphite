...
RUN     mkdir /src                                                                                                   &&\
        git clone --depth=1 --branch 1.1.5 https://github.com/graphite-project/whisper.git /src/whisper             &&\
        cd /src/whisper                                                                                              &&\
        pip install . --no-cache-dir                                                                                 &&\
        python setup.py install

RUN     git clone --depth=1 --branch 1.1.5 https://github.com/graphite-project/carbon.git /src/carbon               &&\
        cd /src/carbon                                                                                               &&\
        pip install . --no-cache-dir                                                                                 &&\
        python setup.py install

RUN     git clone --depth=1 --branch 1.1.5 https://github.com/graphite-project/graphite-web.git /src/graphite-web   &&\
        cd /src/graphite-web                                                                                         &&\
        pip install . --no-cache-dir                                                                                 &&\
        python setup.py install                                                                                      &&\
...
RUN     mkdir /src/grafana                                                                                           &&\
        mkdir /opt/grafana                                                                                           &&\
        curl https://dl.grafana.com/oss/release/grafana-5.4.3.linux-amd64.tar.gz  \
